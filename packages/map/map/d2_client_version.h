...
Act* acts[5] = {nullptr, nullptr, nullptr, nullptr, nullptr};
unsigned int act_seeds[5] = {0,0,0,0,0};
int act_diff[5] = {-1,-1,-1,-1,-1};

Act* d2common_load_act_run(D2Version gameVersion, int actId, unsigned int seed, int difficulty) {
...
Act* d2common_load_act(D2Version gameVersion, int actId, unsigned int seed, int difficulty) {
    ...
    if (acts[actId] != nullptr) d2common_unload_act(gameVersion, acts[actId]);
...
void d2common_add_room_data(D2Version gameVersion, Act* pAct, Level* pLevel, Room2* pRoom2) {
    if (gameVersion == VersionPathOfDiablo) return D2COMMON_Pod_AddRoomData(pAct, pLevel->dwLevelNo, pRoom2->dwPosX, pRoom2->dwPosY, nullptr);
    if (gameVersion == VersionProjectDiablo2) return D2COMMON_Pd2_AddRoomData(pAct, pLevel->dwLevelNo, pRoom2->dwPosX, pRoom2->dwPosY, nullptr);
    if (gameVersion == VersionDiablo2) return D2COMMON_Pd2_AddRoomData(pAct, pLevel->dwLevelNo, pRoom2->dwPosX, pRoom2->dwPosY, nullptr);
}
